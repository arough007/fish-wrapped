...
function __shell_track_pre --on-event fish_preexec
    set -g __shell_track_start $EPOCHREALTIME
end

function __shell_track_post --on-event fish_postexec
    set -l exit_code $status
    set -l end_time $EPOCHREALTIME
    set -l duration 0

    if set -q __shell_track_start
        set duration (math -s0 "($end_time - $__shell_track_start) * 1000")
    end

    set -l cmd (string replace --all -- \n \\n $argv[1])
    set -l pwd_path $PWD
    set -l timestamp (math -s0 "$end_time")
